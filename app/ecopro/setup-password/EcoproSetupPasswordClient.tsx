...
export default function EcoproSetupPasswordClient() {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        const response = await fetch(`/api/ecopro-setup-password/validate-token?token=${encodeURIComponent(token)}`);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const response = await fetch("/api/ecopro-setup-password", {
        method: "POST",
      ...
      setTimeout(() => {
        window.location.href = data.redirectTo || "/ecopro";
      }, 700);
    } catch (err) {
      ...
      <div className="relative z-10 min-h-screen flex items-center justify-center p-6">
        <div className="w-full max-w-[560px]">
          <div className="mb-8 text-center">
            ...
            <p className="text-sm text-sky-100/90 font-medium">Állítsd be a jelszavad az EcoPro dedikált felületéhez.</p>
          </div>
          ...
          <div ref={resultRef} className="rounded-[2rem] border border-white/10 bg-white/95 text-admin-gray-900 shadow-[0_30px_80px_rgba(0,0,0,0.25)] overflow-hidden">
            <div className="h-1.5 bg-[linear-gradient(90deg,#00B4D8_0%,#0096B4_60%,#F28C28_100%)]" />
              ...
              {tokenStatus === "invalid" ? (
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <button
                    ...
                    style={{ background: "linear-gradient(90deg, #00B4D8 0%, #0096B4 60%, #F28C28 100%)" }}
                  >
