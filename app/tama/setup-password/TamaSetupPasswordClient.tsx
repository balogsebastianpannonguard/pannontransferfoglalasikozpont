...
export default function TamaSetupPasswordClient() {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        const response = await fetch(`/api/tama-setup-password/validate-token?token=${encodeURIComponent(token)}`);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const response = await fetch("/api/tama-setup-password", {
        method: "POST",
      ...
      setTimeout(() => {
        window.location.href = data.redirectTo || "/tama";
      }, 700);
    } catch (err) {
  ...
  return (
    <section className="min-h-screen bg-[#2a4a00] text-white relative overflow-hidden">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_left,rgba(92,167,0,0.22),transparent_35%),radial-gradient(circle_at_bottom_right,rgba(67,128,0,0.18),transparent_30%),linear-gradient(180deg,#1f3600_0%,#2a4a00_100%)]" />
      ...
      <div className="relative z-10 min-h-screen flex items-center justify-center p-6">
        <div className="w-full max-w-[560px]">
          <div className="mb-8 text-center">
            ...
            <p className="text-sm text-lime-100/90 font-medium">Állítsd be a jelszavad a Tama dedikált felületéhez.</p>
          </div>
          ...
          <div ref={resultRef} className="rounded-[2rem] border border-white/10 bg-white/95 text-admin-gray-900 shadow-[0_30px_80px_rgba(0,0,0,0.25)] overflow-hidden">
            <div className="h-1.5 bg-[linear-gradient(90deg,#5CA700_0%,#438000_60%,#5CA700_100%)]" />
              ...
              {tokenStatus === "invalid" ? (
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  <div>
                    ...
                    {requireTwoFactor && (
                      <div className="mt-3 inline-flex items-center gap-2 rounded-full border border-lime-200 bg-lime-50 px-3 py-1.5 text-[11px] font-bold uppercase tracking-wider text-lime-700">
                  ...
                  <button
                    ...
                    style={{ background: "linear-gradient(90deg, #5CA700 0%, #438000 60%, #5CA700 100%)" }}
                  >
