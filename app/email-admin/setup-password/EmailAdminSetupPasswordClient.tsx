...
export default function EmailAdminSetupPasswordClient() {
    ...
    (async () => {
      ...
      try {
          const response = await fetch(
            `/api/setup-password/validate-token?token=${encodeURIComponent(token)}&system=email-admin`
          );
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const response = await fetch("/api/email-admin/setup-password", {
        method: "POST",
      ...
      setTimeout(() => {
        window.location.href = data.redirectTo || "/email-admin/login";
      }, 900);
    } catch (err) {
      console.error("[EmailAdmin SetupPassword] Hiba:", err);
      setSubmitError(
  ...
  return (
    <section className="relative min-h-screen w-full bg-[#FAFAFA] text-admin-black font-sans flex items-center justify-center p-6 selection:bg-admin-yellow selection:text-white overflow-hidden">
      ...
      <div className="absolute inset-0 z-0 pointer-events-none">
        ...
        <div
          className="absolute inset-0 opacity-60 mix-blend-multiply animate-float"
          style={{
            background:
              "radial-gradient(circle at 15% 25%, rgba(230,184,0,0.18) 0%, transparent 50%), radial-gradient(circle at 85% 15%, rgba(184,134,11,0.22) 0%, transparent 50%), radial-gradient(circle at 75% 85%, rgba(255,215,0,0.20) 0%, transparent 50%), radial-gradient(circle at 25% 80%, rgba(17,17,17,0.05) 0%, transparent 50%)",
            filter: "blur(70px)",
          }}
        />
      ...
      <div className="relative z-10 w-full max-w-[520px]">
        ...
        >
          <div className="relative inline-flex items-center justify-center w-16 h-16 bg-white/80 backdrop-blur-xl rounded-[1.25rem] shadow-[0_8px_30px_rgba(0,0,0,0.06)] border border-white mb-6 group">
            <div className="absolute inset-0 rounded-[1.25rem] bg-gradient-to-br from-admin-yellow/20 to-[#B8860B]/10 opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
            <div className="relative w-8 h-8 rounded-lg bg-gradient-to-br from-admin-yellow to-[#B8860B] flex items-center justify-center">
          ...
          <p className="text-[11px] font-bold tracking-[0.25em] uppercase bg-gradient-to-r from-[#B8860B] via-admin-yellow to-admin-yellow bg-clip-text text-transparent">
            Pannon Transfer • Email Admin
          </p>
        </div>
        ...
        {tokenStatus === "invalid" && (
          ...
          >
            <div className="absolute inset-x-0 top-0 h-[4px] bg-gradient-to-r from-admin-yellow via-[#E6B800] to-[#B8860B]" />
            <div className="px-8 sm:px-10 py-10">
              <div className="flex items-center justify-center w-16 h-16 mx-auto mb-6 rounded-2xl bg-admin-yellow/15 border border-admin-yellow/20">
              ...
              <button
                ...
                className="w-full rounded-2xl bg-gradient-to-r from-admin-yellow to-[#E6B800] hover:from-[#F7C800] hover:to-[#FFD500] text-admin-gray-900 py-4 text-[11px] font-black tracking-[0.2em] uppercase transition-all shadow-[0_6px_16px_rgba(230,184,0,0.22)]"
              >
        ...
        {tokenStatus !== "invalid" && (
          ...
          >
            ...
            <div className="px-8 sm:px-10 py-10">
              {/* Folyamatjelző - csak egy lépés az Email Adminnál, nincs 2FA */}
              <div className="mb-8 flex items-center justify-center">
                <div className="flex items-center gap-3 px-6 py-3 rounded-full bg-admin-yellow/10 border border-admin-yellow/15">
                  <div className="w-6 h-6 rounded-full bg-admin-yellow flex items-center justify-center shadow-md">
                    <svg className="w-3.5 h-3.5 text-admin-gray-900" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M16.5 10.5V6.75a4.5 4.5 0 10-9 0v3.75m-.75 11.25h10.5a2.25 2.25 0 002.25-2.25v-6.75a2.25 2.25 0 00-2.25-2.25H6.75a2.25 2.25 0 00-2.25 2.25v6.75a2.25 2.25 0 002.25 2.25z" />
                    </svg>
                  </div>
                  <span className="text-[10px] font-black tracking-[0.18em] uppercase text-admin-gray-900">
                    Admin hitelesítés
                  </span>
                </div>
              </div>
              ...
              {tokenStatus === "valid" && !submitSuccess && (
                <form onSubmit={handleSubmit} className="space-y-7">
                  ...
                  <div className="rounded-2xl bg-admin-yellow/5 border border-admin-yellow/15 p-4">
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 rounded-xl bg-white border border-admin-yellow/15 flex items-center justify-center shadow-sm">
                        <svg className="w-4 h-4 text-[#B8860B]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
                          <path strokeLinecap="round" strokeLinejoin="round" d="M21.75 6.75v10.5a2.25 2.25 0 01-2.25 2.25h-15a2.25 2.25 0 01-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25m19.5 0v.243a2.25 2.25 0 01-1.07 1.916l-7.5 4.615a2.25 2.25 0 01-2.36 0L3.32 8.91a2.25 2.25 0 01-1.07-1.916V6.75" />
                        </svg>
                      </div>
                      <div className="min-w-0 flex-1">
                        <div className="text-[10px] font-black tracking-[0.18em] uppercase text-[#B8860B]/70 mb-0.5">
                          Email Admin fiók
                        </div>
                        <div className="text-sm font-bold text-admin-gray-900 truncate">
                          {userEmail || "Meghívott admin"}
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative group">
                      <div className="absolute inset-0 bg-gradient-to-r from-admin-yellow/30 to-[#B8860B]/20 rounded-2xl blur opacity-0 group-focus-within:opacity-100 transition-opacity duration-500" />
                      <input
                        id="password"
                        value={password}
                        onChange={(e) => {
                        ...
                        className="relative w-full bg-white/80 border border-admin-gray-200/60 rounded-2xl pl-5 pr-14 py-4 text-sm text-admin-gray-900 placeholder:text-admin-gray-400 outline-none transition-all duration-300 focus:bg-white focus:border-admin-yellow/60 shadow-[inset_0_2px_4px_rgba(0,0,0,0.02)]"
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative group">
                      ...
                      <input
                        ...
                        className={`relative w-full bg-white/80 rounded-2xl pl-5 pr-14 py-4 text-sm text-admin-gray-900 placeholder:text-admin-gray-400 outline-none transition-all duration-300 shadow-[inset_0_2px_4px_rgba(0,0,0,0.02)] ${passwordsMatch ? "border border-green-500/40 focus:bg-white focus:border-green-500/60" : confirmPassword && !passwordsMatch ? "border border-admin-red/40 focus:bg-white focus:border-admin-red/50" : "border border-admin-gray-200/60 focus:bg-white focus:border-admin-yellow/60"}`}
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        <div className="w-20 h-1.5 bg-admin-gray-200 rounded-full overflow-hidden">
                          <div
                            className="h-full transition-all duration-300 bg-gradient-to-r from-admin-yellow via-[#F7C800] to-green-500"
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-2.5">
                      {(
                        ...
                        <div key={key} className="flex items-center gap-2">
                          <div
                            className={`w-4.5 h-4.5 shrink-0 rounded-full border flex items-center justify-center transition-all duration-300 ${checks[key] ? "bg-admin-yellow border-admin-yellow" : "bg-white border-admin-gray-200"}`}
                          >
                            {checks[key] ? (
                              <svg className="w-2.5 h-2.5 text-admin-gray-900" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={4}>
                  ...
                  <button
                    type="submit"
                    disabled={!canSubmit}
                    className={`group relative w-full rounded-2xl overflow-hidden transition-all duration-300 mt-2 ${canSubmit ? "shadow-[0_8px_20px_rgba(230,184,0,0.22)] hover:shadow-[0_12px_25px_rgba(230,184,0,0.35)] hover:-translate-y-0.5 active:translate-y-0" : "opacity-50 cursor-not-allowed"}`}
                  >
              ...
              {submitSuccess && (
                <div className="py-6">
                  <div className="flex flex-col items-center gap-5 animate-fade-in-up">
                    <div className="w-20 h-20 rounded-2xl bg-admin-yellow/10 border border-admin-yellow/25 flex items-center justify-center">
                      <svg className="w-10 h-10 text-[#B8860B]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                        <path strokeLinecap="round" strokeLinejoin="round" d="M4.5 12.75l6 6 9-13.5" />
                      </svg>
                    </div>
                    ...
                    <p className="text-sm text-admin-gray-600 text-center font-medium">
                      Átirányítás az Email Admin bejelentkező oldalra...
                    </p>
            ...
            <div className="bg-admin-gray-50/50 backdrop-blur-md border-t border-white/40 px-8 py-5 flex items-center justify-center gap-2">
              ...
              <p className="text-[10px] font-bold tracking-wider uppercase text-admin-gray-400">
                Email Admin Hitelesítés • BCrypt
              </p>
            </div>
          </div>
        )}
