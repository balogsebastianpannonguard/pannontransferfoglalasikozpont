...
export default function SchaefflerSetupPasswordClient() {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        const response = await fetch(`/api/schaeffler-setup-password/validate-token?token=${encodeURIComponent(token)}`);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const response = await fetch("/api/schaeffler-setup-password", {
        method: "POST",
      ...
      setTimeout(() => {
        window.location.href = data.redirectTo || "/schaeffler";
      }, 700);
    } catch (err) {
      ...
      <div className="relative z-10 min-h-screen flex items-center justify-center p-6">
        <div className="w-full max-w-[560px]">
          <div className="mb-8 text-center">
            ...
            <p className="text-sm text-emerald-100/90 font-medium">Állítsd be a jelszavad a Schaeffler dedikált felületéhez.</p>
          </div>
          ...
          <div ref={resultRef} className="rounded-[2rem] border border-white/10 bg-white/95 text-admin-gray-900 shadow-[0_30px_80px_rgba(0,0,0,0.25)] overflow-hidden">
            <div className="h-1.5 bg-[linear-gradient(90deg,#009A44_0%,#007A35_60%,#009A44_100%)]" />
              ...
              {tokenStatus === "invalid" ? (
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  <div>
                    ...
                    {requireTwoFactor && (
                      <div className="mt-3 inline-flex items-center gap-2 rounded-full border border-green-200 bg-green-50 px-3 py-1.5 text-[11px] font-bold uppercase tracking-wider text-green-700">
                  ...
                  <button
                    ...
                    style={{ background: "linear-gradient(90deg, #009A44 0%, #007A35 60%, #009A44 100%)" }}
                  >
