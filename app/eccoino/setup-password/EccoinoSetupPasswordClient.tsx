...
export default function EccoinoSetupPasswordClient() {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        const response = await fetch(`/api/eccoino-setup-password/validate-token?token=${encodeURIComponent(token)}`);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const response = await fetch("/api/eccoino-setup-password", {
        method: "POST",
      ...
      setTimeout(() => {
        window.location.href = data.redirectTo || "/eccoino";
      }, 700);
    } catch (err) {
        ...
        <div className="w-full max-w-[560px]">
          <div className="mb-8 text-center">
            <div className="inline-flex items-center gap-2 bg-white rounded-[1.5rem] px-6 py-4 shadow-[0_20px_60px_rgba(0,0,0,0.2)] mb-6">
              <span className="text-[2rem] font-black tracking-tight text-[#60B8FF]">Eccoino</span>
              <span className="text-lg font-extrabold tracking-tight text-[#7B68EE]">Transfer</span>
            </div>
            <h1 className="font-serif text-4xl font-bold tracking-tight mb-2">Partnerportál aktiválás</h1>
            <p className="text-sm text-sky-100/90 font-medium">Állítsd be a jelszavad az Eccoino dedikált felületéhez.</p>
          </div>
          ...
          <div ref={resultRef} className="rounded-[2rem] border border-white/10 bg-white/95 text-admin-gray-900 shadow-[0_30px_80px_rgba(0,0,0,0.25)] overflow-hidden">
            <div className="h-1.5 bg-[linear-gradient(90deg,#60B8FF_0%,#7B68EE_60%,#60B8FF_100%)]" />
              ...
              {tokenStatus === "invalid" ? (
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  <div>
                    ...
                    {requireTwoFactor && (
                      <div className="mt-3 inline-flex items-center gap-2 rounded-full border border-sky-200 bg-sky-50 px-3 py-1.5 text-[11px] font-bold uppercase tracking-wider text-sky-700">
                  ...
                  <button
                    ...
                    style={{ background: "linear-gradient(90deg, #60B8FF 0%, #7B68EE 60%, #60B8FF 100%)" }}
                  >
