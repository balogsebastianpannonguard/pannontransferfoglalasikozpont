...
export default function VitescoSetupPasswordClient() {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        const response = await fetch(`/api/vitesco-setup-password/validate-token?token=${encodeURIComponent(token)}`);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const response = await fetch("/api/vitesco-setup-password", {
        method: "POST",
      ...
      setTimeout(() => {
        window.location.href = data.redirectTo || "/vitesco";
      }, 700);
    } catch (err) {
  ...
  return (
    <section className="min-h-screen bg-[#1a0a0a] text-white relative overflow-hidden">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_left,rgba(227,6,19,0.22),transparent_35%),radial-gradient(circle_at_bottom_right,rgba(184,0,16,0.18),transparent_30%),linear-gradient(180deg,#2a0f0f_0%,#1a0a0a_100%)]" />
        ...
        <div className="w-full max-w-[560px]">
          <div className="mb-8 text-center">
            <div className="inline-flex items-center gap-2 bg-white rounded-[1.5rem] px-6 py-4 shadow-[0_20px_60px_rgba(0,0,0,0.2)] mb-6">
              <span className="text-[2rem] font-black tracking-tight text-[#E30613]">Vitesco</span>
            </div>
            <h1 className="font-serif text-4xl font-bold tracking-tight mb-2">Partnerportál aktiválás</h1>
            <p className="text-sm text-red-100/90 font-medium">Állítsd be a jelszavad a Vitesco dedikált felületéhez.</p>
          </div>
          ...
          <div ref={resultRef} className="rounded-[2rem] border border-white/10 bg-white/95 text-admin-gray-900 shadow-[0_30px_80px_rgba(0,0,0,0.25)] overflow-hidden">
            <div className="h-1.5 bg-[linear-gradient(90deg,#E30613_0%,#B80010_60%,#E30613_100%)]" />
              ...
              {tokenStatus === "invalid" ? (
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  <div>
                    ...
                    {requireTwoFactor && (
                      <div className="mt-3 inline-flex items-center gap-2 rounded-full border border-red-200 bg-red-50 px-3 py-1.5 text-[11px] font-bold uppercase tracking-wider text-red-700">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {rules.map((rule) => (
                        <div key={rule.label} className="flex items-center gap-2">
                          <div className={`w-5 h-5 rounded-full flex items-center justify-center text-white text-xs font-bold ${rule.ok ? "bg-green-500" : "bg-admin-gray-300"}`}>
                            {rule.ok ? "✓" : "−"}
                          </div>
                          <span className={`text-xs ${rule.ok ? "text-admin-gray-700 font-bold" : "text-admin-gray-500"}`}>{rule.label}</span>
                        </div>
                      ))}
                    </div>
                  </div>

                  {submitError && <div className="rounded-2xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm font-semibold text-rose-700">{submitError}</div>}
                  ...
                  <button
                    ...
                    style={{ background: canSubmit ? "linear-gradient(90deg, #E30613 0%, #B80010 60%, #E30613 100%)" : "#999" }}
                  >
                    {isSubmitting ? "Beállítás..." : submitSuccess ? "Sikeres!" : "Jelszó beállítása"}
                  </button>
                </form>
              )}
            </div>
