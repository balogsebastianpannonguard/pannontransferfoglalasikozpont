...
export default function EnterAirBookingsClient() {
  ...
  useEffect(() => {
    setIsLoading(false);
    setUserEmail("partner@enterair.hu");
  }, []);

  const handleLogout = async () => {
    try {
      await fetch("/api/enterair-auth/logout", { method: "POST" });
      router.push("/enterair/login");
    } catch {
      // ignore
    }
  };
      ...
      <div className="relative z-10">
        ...
        <div className="border-b border-white/10 bg-white/5 backdrop-blur-xl">
          <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
            <div className="inline-flex items-center gap-2">
              <span className="text-[1.5rem] font-black tracking-tight text-[#005BAA]">Enter Air</span>
            </div>
            <div className="flex items-center gap-4">
              <div className="text-right">
                <div className="text-xs font-semibold text-cyan-200">Bejelentkezve mint</div>
        ...
        <div className="max-w-7xl mx-auto px-6 py-12">
          <div className="mb-10">
            ...
            <p className="text-cyan-200 text-lg">Kezelje az Enter Air transfer foglalásokat.</p>
          </div>

          {isLoading ? (
            <div className="flex items-center justify-center py-20">
              <div className="text-cyan-200 font-semibold">Betöltés...</div>
            </div>
                ...
                >
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-cyan-200 text-sm">{booking.date}</p>
                    </div>
                    <div className="text-right">
                      <div className="font-bold text-white">{booking.price} EUR</div>
                      <div className="text-cyan-200 text-sm">{booking.status}</div>
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl p-12 text-center">
              <p className="text-cyan-200 text-lg mb-6">Nincsenek aktív foglalások.</p>
              <Link
                href="/enterair"
                className="h-12 px-6 rounded-2xl bg-[#005BAA] text-white text-sm font-black tracking-widest uppercase inline-flex items-center justify-center hover:bg-[#0078D4] transition-colors"
              >
                Vissza a főoldalra
              </Link>
            </div>
