...
export default function EnterAirSetupPasswordClient() {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        const response = await fetch(`/api/enterair-setup-password/validate-token?token=${encodeURIComponent(token)}`);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const response = await fetch("/api/enterair-setup-password", {
        method: "POST",
      ...
      <div className="relative z-10 min-h-screen flex items-center justify-center p-6">
        <div className="w-full max-w-[560px]">
          <div className="mb-8 text-center">
            ...
            <p className="text-sm text-cyan-100/90 font-medium">Állítsd be a jelszavad az Enter Air dedikált felületéhez.</p>
          </div>
              ...
              {tokenStatus === "invalid" ? (
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  <div>
                    ...
                    {requireTwoFactor && (
                      <div className="mt-3 inline-flex items-center gap-2 rounded-full border border-cyan-200 bg-cyan-50 px-3 py-1.5 text-[11px] font-bold uppercase tracking-wider text-cyan-700">
