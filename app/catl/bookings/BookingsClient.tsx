...
export default function CatlBookingsClient() {
  ...
  useEffect(() => {
    setIsLoading(false);
    setUserEmail("partner@catl.hu");
  }, []);

  const handleLogout = async () => {
    try {
      await fetch("/api/catl-auth/logout", { method: "POST" });
      router.push("/catl/login");
    } catch {
      // ignore
    }
  };
      ...
      <div className="relative z-10">
        ...
        <div className="border-b border-white/10 bg-white/5 backdrop-blur-xl">
          <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
            <div className="inline-flex items-center gap-2">
              <span className="text-[1.5rem] font-black tracking-tight text-[#0047BA]">CATL</span>
            </div>
        ...
        <div className="max-w-7xl mx-auto px-6 py-12">
          <div className="mb-10">
            ...
            <p className="text-sky-200 text-lg">Kezelje a CATL transfer foglalásokat.</p>
          </div>
          ...
          {isLoading ? (
            ...
            <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl p-12 text-center">
              ...
              <Link
                href="/catl"
                className="h-12 px-6 rounded-2xl bg-[#0047BA] text-white text-sm font-black tracking-widest uppercase inline-flex items-center justify-center hover:bg-[#003390] transition-colors"
              >
                Vissza a főoldalra
              </Link>
            </div>
