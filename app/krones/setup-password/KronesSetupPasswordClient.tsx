...
export default function KronesSetupPasswordClient() {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        const response = await fetch(`/api/krones-setup-password/validate-token?token=${encodeURIComponent(token)}`);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const response = await fetch("/api/krones-setup-password", {
        method: "POST",
      ...
      setTimeout(() => {
        window.location.href = data.redirectTo || "/krones";
      }, 700);
    } catch (err) {
  ...
  return (
    <section className="min-h-screen bg-[#001a40] text-white relative overflow-hidden">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_left,rgba(0,63,138,0.22),transparent_35%),radial-gradient(circle_at_bottom_right,rgba(0,45,106,0.18),transparent_30%),linear-gradient(180deg,#000f24_0%,#001a40_100%)]" />
      ...
      <div className="relative z-10 min-h-screen flex items-center justify-center p-6">
        <div className="w-full max-w-[560px]">
          <div className="mb-8 text-center">
            ...
            <p className="text-sm text-blue-100/90 font-medium">Állítsd be a jelszavad a Krones dedikált felületéhez.</p>
          </div>
          ...
          <div ref={resultRef} className="rounded-[2rem] border border-white/10 bg-white/95 text-admin-gray-900 shadow-[0_30px_80px_rgba(0,0,0,0.25)] overflow-hidden">
            <div className="h-1.5 bg-[linear-gradient(90deg,#003F8A_0%,#002D6A_60%,#003F8A_100%)]" />
              ...
              {tokenStatus === "invalid" ? (
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  <div>
                    ...
                    {requireTwoFactor && (
                      <div className="mt-3 inline-flex items-center gap-2 rounded-full border border-blue-200 bg-blue-50 px-3 py-1.5 text-[11px] font-bold uppercase tracking-wider text-blue-700">
                  ...
                  <button
                    ...
                    style={{ background: "linear-gradient(90deg, #003F8A 0%, #002D6A 60%, #003F8A 100%)" }}
                  >
