...
import { createOrResetSchaefflerInvite } from "@/lib/schaeffler-portal-users";
...
<tr><td bgcolor="#009A44" style="padding:44px 40px; border-bottom:3px solid #007335;">
<table role="presentation" cellpadding="0" cellspacing="0" border="0" style="margin-bottom:22px;"><tr>
<td align="left" valign="middle" bgcolor="#FFFFFF" style="border-radius:16px; padding:16px 20px;">
<span style="font-family:-apple-system, sans-serif; font-size:18px; font-weight:900; color:#009A44;">Schaeffler</span>
<span style="font-family:-apple-system, sans-serif; font-size:14px; font-weight:800; color:#007335; margin-left:6px;">Partner</span>
</td></tr></table>
<h1 style="margin:0 0 8px 0; font-family:Georgia, serif; color:#FFFFFF; font-size:26px; letter-spacing:.5px;">Schaeffler Partner Portál</h1>
<p style="margin:0; font-size:11px; font-weight:700; letter-spacing:3px; text-transform:uppercase; color:#A0E7A0;">Pannon Transfer Partner</p>
</td></tr>
...
Meghívást kaptál a Schaeffler dedikált portálra. Kizárólag az alábbi egyedi linken keresztül tudsz belépni és elérni az árstruktúrát és a foglalásokat.
</p>
...
Ezt az üzenetet a Pannon Transfer Schaeffler Portál rendszere küldte. Ha nem kérted a meghívást, hagyd figyelmen kívül.<br>
© ${new Date().getFullYear()} Pannon Transfer. Minden jog fenntartva.
  ...
  const text = [
    "Schaeffler Portál - Pannon Transfer",
    "",
    "Kedves Partner!",
    "",
    "Meghívást kaptál a Schaeffler dedikált portálra. Kizárólag az alábbi linken keresztül tudsz belépni:",
    setupUrl,
    "",
...
export async function POST(request: Request) {
  try {
    ...
    if (!smtp.success) {
      console.warn("[SMTP] Schaeffler invite: SMTP nem érhető el (teszt mód).", smtp.message);
    }
    ...
    const setupPath = "/schaeffler/setup-password";
    ...
    for (const recipient of recipientList) {
      try {
        const { user, rawToken } = await createOrResetSchaefflerInvite(recipient, {
        ...
        const sendRes = await sendEmail({
          to: recipient,
          subject: "Meghívás a Schaeffler Partner Portálra – Pannon Transfer",
          html,
          text,
        });
    ...
    console.error("[schaeffler-invites/send] error", error);
    return NextResponse.json(
