...
import {
  ...
  setKronesUserPasswordAndActivate,
...
export async function POST(request: Request) {
  try {
      ...
      return NextResponse.json(
        {
          success: false,
          code: "INVALID_TOKEN",
          message: "Érvénytelen vagy lejárt Krones meghívó link.",
        },
        { status: 400 }
      );
    }
    ...
    await setKronesUserPasswordAndActivate(new ObjectId(String(user._id)), password);

    return NextResponse.json({
      success: true,
      message: "A Krones portál jelszava sikeresen beállítva.",
      redirectTo: "/krones?activated=1",
