...
import { createOrResetVitescoInvite } from "@/lib/vitesco-portal-users";
...
<tr><td bgcolor="#E30613" style="padding:44px 40px; border-bottom:3px solid #B80010;">
...
<span style="font-family:-apple-system, sans-serif; font-size:18px; font-weight:900; color:#E30613;">Vitesco</span>
<span style="font-family:-apple-system, sans-serif; font-size:14px; font-weight:800; color:#B80010; margin-left:6px;">Technologies</span>
</td></tr></table>
<h1 style="margin:0 0 8px 0; font-family:Georgia, serif; color:#FFFFFF; font-size:26px; letter-spacing:.5px;">Vitesco Partner Portál</h1>
<p style="margin:0; font-size:11px; font-weight:700; letter-spacing:3px; text-transform:uppercase; color:#FFD700;">Pannon Transfer Partner</p>
</td></tr>
...
Meghívást kaptál a Vitesco dedikált portálra. Kizárólag az alábbi egyedi linken keresztül tudsz belépni és elérni az árstruktúrát és a foglalásokat.
</p>
...
Ezt az üzenetet a Pannon Transfer Vitesco Portál rendszere küldte. Ha nem kérted a meghívást, hagyd figyelmen kívül.<br>
© ${new Date().getFullYear()} Pannon Transfer. Minden jog fenntartva.
  ...
  const text = [
    "Vitesco Portál - Pannon Transfer",
    "",
    "Kedves Partner!",
    "",
    "Meghívást kaptál a Vitesco dedikált portálra. Kizárólag az alábbi linken keresztül tudsz belépni:",
    setupUrl,
    "",
...
export async function POST(request: Request) {
  try {
    ...
    if (!smtp.success) {
      console.warn("[SMTP] Vitesco invite: SMTP nem érhető el (teszt mód).", smtp.message);
    }
    ...
    const setupPath = "/vitesco/setup-password";
    ...
    for (const recipient of recipientList) {
      try {
        const { user, rawToken } = await createOrResetVitescoInvite(recipient, {
        ...
        const sendRes = await sendEmail({
          to: recipient,
          subject: "Meghívás a Vitesco Partner Portálra – Pannon Transfer",
          html,
          text,
        });
    ...
    console.error("[vitesco-invites/send] error", error);
    return NextResponse.json(
