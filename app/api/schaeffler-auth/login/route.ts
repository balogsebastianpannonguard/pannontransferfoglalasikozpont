...
import {
  ...
  recordSchaefflerSuccessfulLogin,
...
export async function POST(request: Request) {
  try {
    ...
    let user = await findSchaefflerUserByEmail(userEmail);
    ...
    const passwordOk = await compareSchaefflerPassword(user, password.trim());
    if (!passwordOk) {
      return NextResponse.json(
        { success: false, message: "Hibás bejelentkezési adatok." },
        { status: 401 }
    ...
    await recordSchaefflerSuccessfulLogin(user._id as any as ObjectId);
    ...
    const AUTH_COOKIE_NAME = "schaeffler_portal_session";
