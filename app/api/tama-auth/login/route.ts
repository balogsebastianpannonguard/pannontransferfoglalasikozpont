...
export async function POST(request: Request) {
  try {
    ...
    let user = await findTamaUserByEmail(userEmail);
    ...
    const passwordOk = await compareTamaPassword(user, password.trim());
    if (!passwordOk) {
      return NextResponse.json(
        { success: false, message: "Hibás bejelentkezési adatok." },
        { status: 401 }
    ...
    await recordTamaSuccessfulLogin(user._id as any as ObjectId);
    ...
    const AUTH_COOKIE_NAME = "tama_portal_session";
