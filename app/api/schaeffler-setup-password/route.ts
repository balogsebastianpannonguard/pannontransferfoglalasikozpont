...
import {
  ...
  setSchaefflerUserPasswordAndActivate,
...
export async function POST(request: Request) {
  try {
      ...
      return NextResponse.json(
        {
          success: false,
          code: "INVALID_TOKEN",
          message: "Érvénytelen vagy lejárt Schaeffler meghívó link.",
        },
        { status: 400 }
      );
    }
    ...
    await setSchaefflerUserPasswordAndActivate(new ObjectId(String(user._id)), password);

    return NextResponse.json({
      success: true,
      message: "A Schaeffler portál jelszava sikeresen beállítva.",
      redirectTo: "/schaeffler?activated=1",
