...
import { createOrResetEnterAirInvite } from "@/lib/enterair-portal-users";
...
<tr><td bgcolor="#005BAA" style="padding:44px 40px; border-bottom:3px solid #004480;">
...
<span style="font-family:-apple-system, sans-serif; font-size:18px; font-weight:900; color:#005BAA;">EnterAir</span>
<span style="font-family:-apple-system, sans-serif; font-size:14px; font-weight:800; color:#004480; margin-left:6px;">Travel</span>
</td></tr></table>
<h1 style="margin:0 0 8px 0; font-family:Georgia, serif; color:#FFFFFF; font-size:26px; letter-spacing:.5px;">EnterAir Partner Portál</h1>
<p style="margin:0; font-size:11px; font-weight:700; letter-spacing:3px; text-transform:uppercase; color:#5DADE2;">Pannon Transfer Partner</p>
</td></tr>
...
Meghívást kaptál az EnterAir dedikált portálra. Kizárólag az alábbi egyedi linken keresztül tudsz belépni és elérni az árstruktúrát és a foglalásokat.
</p>
...
Ezt az üzenetet a Pannon Transfer EnterAir Portál rendszere küldte. Ha nem kérted a meghívást, hagyd figyelmen kívül.<br>
© ${new Date().getFullYear()} Pannon Transfer. Minden jog fenntartva.
  ...
  const text = [
    "EnterAir Portál - Pannon Transfer",
    "",
    "Kedves Partner!",
    "",
    "Meghívást kaptál az EnterAir dedikált portálra. Kizárólag az alábbi linken keresztül tudsz belépni:",
    setupUrl,
    "",
...
export async function POST(request: Request) {
  try {
    ...
    if (!smtp.success) {
      console.warn("[SMTP] EnterAir invite: SMTP nem érhető el (teszt mód).", smtp.message);
    }
    ...
    const setupPath = "/enterair/setup-password";
    ...
    for (const recipient of recipientList) {
      try {
        const { user, rawToken } = await createOrResetEnterAirInvite(recipient, {
        ...
        const sendRes = await sendEmail({
          to: recipient,
          subject: "Meghívás az EnterAir Partner Portálra – Pannon Transfer",
          html,
          text,
        });
    ...
    console.error("[enterair-invites/send] error", error);
    return NextResponse.json(
