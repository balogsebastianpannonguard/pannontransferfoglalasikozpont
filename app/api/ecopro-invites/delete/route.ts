...
import { deleteEcoproPortalUser } from "@/lib/ecopro-portal-users";
...
export async function POST(request: Request) {
  try {
    ...
    const ok = await deleteEcoproPortalUser(new ObjectId(id));
    if (!ok) {
      return NextResponse.json(
    ...
    return NextResponse.json({
      success: true,
      message: "EcoPro felhasználó és a hozzá tartozó meghívó sikeresen törölve.",
    });
  } catch (error) {
    return NextResponse.json(
      {
