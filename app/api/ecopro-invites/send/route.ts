...
import { createOrResetEcoproInvite } from "@/lib/ecopro-portal-users";
...
<tr><td bgcolor="#0B1F47" style="padding:44px 40px; border-bottom:3px solid #00B4D8;">
...
<span style="font-family:-apple-system, sans-serif; font-size:18px; font-weight:900; color:#0096D6;">EcoPro</span>
<span style="font-family:-apple-system, sans-serif; font-size:14px; font-weight:800; color:#F28C28; margin-left:6px;">BM Hungary</span>
</td></tr></table>
<h1 style="margin:0 0 8px 0; font-family:Georgia, serif; color:#FFFFFF; font-size:26px; letter-spacing:.5px;">EcoPro Partner Portál</h1>
<p style="margin:0; font-size:11px; font-weight:700; letter-spacing:3px; text-transform:uppercase; color:#7DD3FC;">Pannon Transfer Partner</p>
</td></tr>
...
Meghívást kaptál az EcoPro dedikált portálra. Kizárólag az alábbi egyedi linken keresztül tudsz belépni és elérni az árstruktúrát és a foglalásokat.
</p>
...
Ezt az üzenetet a Pannon Transfer EcoPro Portál rendszere küldte. Ha nem kérted a meghívást, hagyd figyelmen kívül.<br>
© ${new Date().getFullYear()} Pannon Transfer. Minden jog fenntartva.
  ...
  const text = [
    "EcoPro Portál - Pannon Transfer",
    "",
    "Kedves Partner!",
    "",
    "Meghívást kaptál az EcoPro dedikált portálra. Kizárólag az alábbi linken keresztül tudsz belépni:",
    setupUrl,
    "",
...
export async function POST(request: Request) {
  try {
    ...
    if (!smtp.success) {
      console.warn("[SMTP] EcoPro invite: SMTP nem érhető el (teszt mód).", smtp.message);
    }
    ...
    const setupPath = "/ecopro/setup-password";
    ...
    for (const recipient of recipientList) {
      try {
        const { user, rawToken } = await createOrResetEcoproInvite(recipient, {
        ...
        const sendRes = await sendEmail({
          to: recipient,
          subject: "Meghívás az EcoPro Partner Portálra – Pannon Transfer",
          html,
          text,
        });
    ...
    console.error("[ecopro-invites/send] error", error);
    return NextResponse.json(
