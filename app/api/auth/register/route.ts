...
import { PASSWORD_BCRYPT_ROUNDS, normalizeEmail, type CrmUser } from "@/lib/users";
import { sendEmail } from "@/lib/email";

function buildCrmWelcomeEmailHtml(email: string, password: string, loginUrl: string) {
  ...
  return `
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 40px 24px; color: #111;">
      <div style="text-align: center; margin-bottom: 36px;">
        ...
        <h1 style="margin: 0 0 8px; font-size: 28px; font-weight: 800; letter-spacing: -0.02em; font-family: Georgia, 'Times New Roman', serif;">CRM fiókod elkészült</h1>
        <p style="margin: 0; font-size: 12px; font-weight: 800; letter-spacing: 0.3em; text-transform: uppercase; color: #888;">Pannon Transfer • CRM Panel</p>
      </div>

      <div style="background: #FAFAFA; border: 1px solid #F0F0F0; border-radius: 24px; padding: 32px; margin-bottom: 32px;">
        <p style="margin: 0 0 24px; font-size: 15px; color: #333; line-height: 1.6;">
          Sikeresen létrehoztunk neked egy hozzáférést a <strong style="color: #E50914;">Pannon Transfer CRM Panel</strong>-jéhez.
        ...
        <p style="margin: 0; font-size: 13px; color: #666; line-height: 1.6;">
          ...
          A kétfaktoros hitelesítés (2FA) csak akkor lesz kötelező, ha azt külön beállítjuk a fiókodhoz.
        </p>
      </div>

      <div style="text-align: center;">
        <a href="${loginUrl}" style="display: inline-block; background: #111; color: #fff; text-decoration: none; padding: 16px 28px; border-radius: 18px; font-size: 12px; font-weight: 900; letter-spacing: 0.25em; text-transform: uppercase; box-shadow: 0 8px 20px rgba(0,0,0,0.12);">
          Belépés a CRM-be →
        </a>
      </div>
...
function buildCrmWelcomeEmailText(email: string, password: string, loginUrl: string) {
  const year = new Date().getFullYear();
  return `
PANNON TRANSFER – CRM FIÓK ELKÉSZÜLT
=====================================

Sikeresen létrehoztunk neked egy hozzáférést a Pannon Transfer CRM Panel-jéhez.
...
• Belépés:  ${loginUrl}
  ...
  A kétfaktoros hitelesítés (2FA) csak akkor lesz kötelező, ha külön beállítjuk a fiókodhoz.
...
export async function POST(req: Request) {
  try {
    ...
    if (!email || !password) {
      return NextResponse.json({ success: false, message: "Hiányzó e-mail vagy jelszó." }, { status: 400 });
    }

    const collection = await getCollection<CrmUser>("users");
    const normalizedEmail = normalizeEmail(email);

    const existing = await collection.findOne({ normalizedEmail });
    if (existing) {
      return NextResponse.json({ success: false, message: "Ez az e-mail cím már regisztrálva van a CRM-ben." }, { status: 400 });
    }
    ...
    // Welcome email küldés (non-blocking)
    const loginUrl = `${process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000"}/login`;
    const html = buildCrmWelcomeEmailHtml(email.trim(), password, loginUrl);
    const text = buildCrmWelcomeEmailText(email.trim(), password, loginUrl);

    sendEmail({
      to: email.trim(),
      subject: "Pannon Transfer – CRM fiókod elkészült",
      html,
      text,
    }).catch((e) => console.error("[CRM Register] Welcome email küldés sikertelen:", e));

    return NextResponse.json({ success: true, message: "CRM felhasználó sikeresen létrehozva. A belépési adatok elküldésre kerültek az e-mail címre." });
  } catch (error) {
    console.error("[CRM Register] Hiba:", error);
