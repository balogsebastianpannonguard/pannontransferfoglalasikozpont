...
import {
  ...
  setTamaUserPasswordAndActivate,
...
export async function POST(request: Request) {
  try {
      ...
      return NextResponse.json(
        {
          success: false,
          code: "INVALID_TOKEN",
          message: "Érvénytelen vagy lejárt Tama meghívó link.",
        },
        { status: 400 }
      );
    }
    ...
    await setTamaUserPasswordAndActivate(new ObjectId(String(user._id)), password);

    return NextResponse.json({
      success: true,
      message: "A Tama portál jelszava sikeresen beállítva.",
      redirectTo: "/tama?activated=1",
