...
export async function POST(request: Request) {
  try {
    ...
    let user = await findCatlUserByEmail(userEmail);
    ...
    // 3. Jelszó ellenőrzés
    const passwordOk = await compareCatlPassword(user, password.trim());
    if (!passwordOk) {
      return NextResponse.json(
        { success: false, message: "Hibás bejelentkezési adatok." },
        { status: 401 }
    ...
    // 6. Sikeres login
    await recordCatlSuccessfulLogin(user._id as any as ObjectId);
    ...
    const AUTH_COOKIE_NAME = "catl_portal_session";
