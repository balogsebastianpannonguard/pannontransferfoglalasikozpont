...
import {
  ...
  setEccoinoUserPasswordAndActivate,
...
export async function POST(request: Request) {
  try {
    ...
    const user = await findEccoinoUserByInviteToken(token);
    if (!user || !user._id) {
      return NextResponse.json(
        {
          success: false,
          code: "INVALID_TOKEN",
          message: "Érvénytelen vagy lejárt Eccoino meghívó link.",
        },
        { status: 400 }
      );
    }
    ...
    await setEccoinoUserPasswordAndActivate(new ObjectId(String(user._id)), password);

    return NextResponse.json({
      success: true,
      message: "Az Eccoino portál jelszava sikeresen beállítva.",
      redirectTo: "/eccoino?activated=1",
