...
function buildDriverInviteEmail(
  recipientEmail: string,
  setupUrl: string,
  expiresAt: number,
  inviteName?: string
...
<table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" bgcolor="#F8FAFC"><tr><td align="center" style="padding:40px 15px;">
<table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="max-width:600px; background:#FFFFFF; border-radius:16px; overflow:hidden; border:1px solid #E2E8F0; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.05), 0 2px 4px -1px rgba(0, 0, 0, 0.03);">
<tr><td bgcolor="#0B1A2A" style="padding:48px 44px; text-align:center; border-bottom:4px solid #C9A962;">
<table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="margin-bottom:20px;"><tr>
<td align="center">
  <div style="background:#132842; width:72px; height:72px; border-radius:20px; display:inline-block; line-height:72px; text-align:center; border:2px solid rgba(201,169,98,0.3);">
    <span style="font-size:32px;">🚗</span>
  </div>
</td></tr></table>
<h1 style="margin:0 0 8px 0; font-family:system-ui, -apple-system, sans-serif; color:#FFFFFF; font-size:28px; font-weight:800; letter-spacing:-0.5px;">Pannon Transfer</h1>
<p style="margin:0; font-size:12px; font-weight:700; letter-spacing:4px; text-transform:uppercase; color:#C9A962;">Sofőr Modul Hozzáférés</p>
</td></tr>
<tr><td style="padding:46px 44px;">
<h2 style="margin:0 0 20px 0; font-family:system-ui, -apple-system, sans-serif; font-size:22px; color:#0F172A; font-weight:700;">Kedves ${displayName}!</h2>
<p style="margin:0 0 32px 0; font-size:15px; line-height:1.75; color:#475569;">
Sikeresen hozzáadtunk a Pannon Transfer sofőr rendszeréhez. A fiókodat az alábbi gombra kattintva tudod aktiválni, ahol beállíthatod a belépéshez szükséges jelszavadat. Ezt követően a Sofőr Modulon keresztül fogod tudni kezelni a fuvarjaidat.
</p>
<table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="margin-bottom:36px;"><tr><td align="center">
<table role="presentation" cellpadding="0" cellspacing="0" border="0"><tr>
<td align="center" bgcolor="#0B1A2A" style="border-radius:12px; border: 1px solid #132842;">
<a href="${setupUrl}" style="display:inline-block; padding:18px 40px; font-size:14px; font-weight:700; color:#FFFFFF; text-decoration:none; letter-spacing:1px; text-transform:uppercase; border-radius:12px;">
Fiók aktiválása &amp; Jelszó megadása
</a>
</td></tr></table>
</td></tr></table>
<table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="background:#F8FAFC; border-radius:12px; padding:24px; margin-bottom:10px;"><tr>
<td width="50%" style="padding-right:20px;">
<p style="margin:0 0 6px 0; font-size:11px; font-weight:700; letter-spacing:1px; text-transform:uppercase; color:#64748B;">Belépési azonosító (E-mail)</p>
<p style="margin:0; font-size:14px; font-weight:600; color:#0F172A; word-break:break-all;">${recipientEmail}</p>
</td>
<td width="50%" style="padding-left:20px; border-left:1px solid #E2E8F0;">
<p style="margin:0 0 6px 0; font-size:11px; font-weight:700; letter-spacing:1px; text-transform:uppercase; color:#64748B;">Link érvényessége</p>
<p style="margin:0; font-size:14px; font-weight:600; color:#0F172A;">${expiresStr}</p>
</td>
</tr></table>
</td></tr>
<tr><td bgcolor="#F1F5F9" style="padding:32px 44px; border-top:1px solid #E2E8F0;">
<p style="margin:0; font-size:12px; line-height:1.7; color:#64748B; text-align:center;">
Ezt az üzenetet a Pannon Transfer Sofőr rendszere küldte.<br>
Ha nem vártál ilyen meghívót, kérjük hagyd figyelmen kívül.<br>
<span style="color:#C9A962; font-weight:600; margin-top:10px; display:inline-block;">© ${new Date().getFullYear()} Pannon Transfer - Minden jog fenntartva.</span>
</p>
</td></tr>
</table></td></tr></table>
</body></html>
  ...
  const text = [
    "Pannon Transfer - Sofőr Hozzáférés",
    "",
    `Kedves ${displayName}!`,
    "",
    "Sikeresen hozzáadtunk a Pannon Transfer sofőr rendszeréhez. A fiókodat az alábbi linken tudod aktiválni:",
    setupUrl,
    "",
    "Belépési fiókod: " + recipientEmail,
    "Link érvényessége: " + expiresStr,
...
export async function POST(request: Request) {
  try {
    ...
    const { recipients, loginBaseUrl, name } = body || {};
    const staffRole: StaffRole = "driver";
    ...
    if (!smtp.success) {
      console.warn(
        `[SMTP] Driver invite: SMTP nem érhető el (teszt mód).`,
        smtp.message
      );
    }

    // Default to port 3003 for drivers app
    let finalBase =
    ...
    if (finalBase) {
      try {
        ...
        if (
          ...
          (u.port === "3000" || u.port === "3001" || u.port === "3002")
        ) {
          u.port = "3003";
          finalBase = u.origin;
        }
    ...
    for (const recipient of recipientList) {
      try {
        const { user, rawToken } = await createOrResetStaffInvite(recipient, {
          requireTwoFactor: false, // Drivers don't need 2FA according to prompt
          role: staffRole,
          name: typeof name === "string" ? name : undefined,
        });

        const setupUrl = `${finalBase}${setupPath}?token=${encodeURIComponent(rawToken)}&role=${staffRole}`;

        console.log(`\n=== DRIVER INVITE (TEST MODE) ===`);
        console.log("Címzett:", recipient);
        console.log("Szerepkör:", staffRole);
        console.log("2FA kötelező: false");
        console.log("Setup link (kattintva):", setupUrl);
        console.log("Lejár:", new Date(user.inviteExpiresAt).toLocaleString("hu-HU"));
        console.log("================================\n");

        const emailBuilder = buildDriverInviteEmail;
        const subject = "Pannon Transfer - Sofőr Hozzáférés";
