...
import {
  ...
  setEcoproUserPasswordAndActivate,
...
export async function POST(request: Request) {
  try {
      ...
      return NextResponse.json(
        {
          success: false,
          code: "INVALID_TOKEN",
          message: "Érvénytelen vagy lejárt EcoPro meghívó link.",
        },
        { status: 400 }
      );
    }
    ...
    await setEcoproUserPasswordAndActivate(new ObjectId(String(user._id)), password);

    return NextResponse.json({
      success: true,
      message: "Az EcoPro portál jelszava sikeresen beállítva.",
      redirectTo: "/ecopro?activated=1",
