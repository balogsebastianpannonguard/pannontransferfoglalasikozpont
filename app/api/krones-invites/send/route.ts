...
import { createOrResetKronesInvite } from "@/lib/krones-portal-users";
...
<tr><td bgcolor="#003F8A" style="padding:44px 40px; border-bottom:3px solid #002E5F;">
...
<span style="font-family:-apple-system, sans-serif; font-size:18px; font-weight:900; color:#003F8A;">Krones</span>
<span style="font-family:-apple-system, sans-serif; font-size:14px; font-weight:800; color:#002E5F; margin-left:6px;">Group</span>
</td></tr></table>
<h1 style="margin:0 0 8px 0; font-family:Georgia, serif; color:#FFFFFF; font-size:26px; letter-spacing:.5px;">Krones Partner Portál</h1>
<p style="margin:0; font-size:11px; font-weight:700; letter-spacing:3px; text-transform:uppercase; color:#5599DD;">Pannon Transfer Partner</p>
</td></tr>
...
Meghívást kaptál a Krones dedikált portálra. Kizárólag az alábbi egyedi linken keresztül tudsz belépni és elérni az árstruktúrát és a foglalásokat.
</p>
...
Ezt az üzenetet a Pannon Transfer Krones Portál rendszere küldte. Ha nem kérted a meghívást, hagyd figyelmen kívül.<br>
© ${new Date().getFullYear()} Pannon Transfer. Minden jog fenntartva.
  ...
  const text = [
    "Krones Portál - Pannon Transfer",
    "",
    "Kedves Partner!",
    "",
    "Meghívást kaptál a Krones dedikált portálra. Kizárólag az alábbi linken keresztül tudsz belépni:",
    setupUrl,
    "",
...
export async function POST(request: Request) {
  try {
    ...
    if (!smtp.success) {
      console.warn("[SMTP] Krones invite: SMTP nem érhető el (teszt mód).", smtp.message);
    }
    ...
    const setupPath = "/krones/setup-password";
    ...
    for (const recipient of recipientList) {
      try {
        const { user, rawToken } = await createOrResetKronesInvite(recipient, {
        ...
        const sendRes = await sendEmail({
          to: recipient,
          subject: "Meghívás a Krones Partner Portálra – Pannon Transfer",
          html,
          text,
        });
    ...
    console.error("[krones-invites/send] error", error);
    return NextResponse.json(
