...
export async function POST(request: Request) {
  try {
    ...
    let user = await findVitescoUserByEmail(userEmail);
    ...
    const passwordOk = await compareVitescoPassword(user, password.trim());
    if (!passwordOk) {
      return NextResponse.json(
        { success: false, message: "Hibás bejelentkezési adatok." },
        { status: 401 }
    ...
    await recordVitescoSuccessfulLogin(user._id as any as ObjectId);
    ...
    const AUTH_COOKIE_NAME = "vitesco_portal_session";
