...
export async function POST(request: Request) {
  try {
    ...
    let user = await findNiUserByEmail(userEmail);
    ...
    const passwordOk = await compareNiPassword(user, password.trim());
    if (!passwordOk) {
      return NextResponse.json(
        { success: false, message: "Hibás bejelentkezési adatok." },
        { status: 401 }
    ...
    await recordNiSuccessfulLogin(user._id as any as ObjectId);
    ...
    const AUTH_COOKIE_NAME = "ni_portal_session";
