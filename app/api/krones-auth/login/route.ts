...
export async function POST(request: Request) {
  try {
    ...
    let user = await findKronesUserByEmail(userEmail);
    ...
    const passwordOk = await compareKronesPassword(user, password.trim());
    if (!passwordOk) {
      return NextResponse.json(
        { success: false, message: "Hibás bejelentkezési adatok." },
        { status: 401 }
    ...
    await recordKronesSuccessfulLogin(user._id as any as ObjectId);
    ...
    const AUTH_COOKIE_NAME = "krones_portal_session";
