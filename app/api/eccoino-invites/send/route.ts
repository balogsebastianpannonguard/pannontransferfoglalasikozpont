...
import { createOrResetEccoinoInvite } from "@/lib/eccoino-portal-users";
...
<tr><td bgcolor="#0a2847" style="padding:44px 40px; border-bottom:3px solid #60B8FF;">
...
<span style="font-family:-apple-system, sans-serif; font-size:18px; font-weight:900; color:#60B8FF;">Eccoino</span>
<span style="font-family:-apple-system, sans-serif; font-size:14px; font-weight:800; color:#7B68EE; margin-left:6px;">Transfer</span>
</td></tr></table>
<h1 style="margin:0 0 8px 0; font-family:Georgia, serif; color:#FFFFFF; font-size:26px; letter-spacing:.5px;">Eccoino Partner Portál</h1>
<p style="margin:0; font-size:11px; font-weight:700; letter-spacing:3px; text-transform:uppercase; color:#60B8FF;">Pannon Transfer Partner</p>
</td></tr>
...
Meghívást kaptál az Eccoino dedikált portálra. Kizárólag az alábbi egyedi linken keresztül tudsz belépni és elérni az árstruktúrát és a foglalásokat.
</p>
...
Ezt az üzenetet a Pannon Transfer Eccoino Portál rendszere küldte. Ha nem kérted a meghívást, hagyd figyelmen kívül.<br>
© ${new Date().getFullYear()} Pannon Transfer. Minden jog fenntartva.
  ...
  const text = [
    "Eccoino Portál - Pannon Transfer",
    "",
    "Kedves Partner!",
    "",
    "Meghívást kaptál az Eccoino dedikált portálra. Kizárólag az alábbi linken keresztül tudsz belépni:",
    setupUrl,
    "",
...
export async function POST(request: Request) {
  try {
    ...
    if (!smtp.success) {
      console.warn("[SMTP] Eccoino invite: SMTP nem érhető el (teszt mód).", smtp.message);
    }
    ...
    const setupPath = "/eccoino/setup-password";
    ...
    for (const recipient of recipientList) {
      try {
        const { user, rawToken } = await createOrResetEccoinoInvite(recipient, {
        ...
        const sendRes = await sendEmail({
          to: recipient,
          subject: "Meghívás az Eccoino Partner Portálra – Pannon Transfer",
          html,
          text,
        });
    ...
    console.error("[eccoino-invites/send] error", error);
    return NextResponse.json(
